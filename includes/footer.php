    <!-- Footer -->
    <footer class="footer">
        <div class="container">
            <div class="footer-content">
                <div class="footer-brand">
                    <div class="logo">
                        <i class="fas fa-industry"></i>
                        <span class="logo-text">OS<span class="accent">CON</span></span>
                    </div>
                    <p>Excelencia en fabricación y soldadura industrial</p>
                    <div class="social-links">
                        <a href="https://facebook.com" target="_blank" rel="noopener noreferrer" aria-label="Facebook"><i class="fab fa-facebook"></i></a>
                        <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" aria-label="Instagram"><i class="fab fa-instagram"></i></a>
                        <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"><i class="fab fa-linkedin"></i></a>
                        <a href="https://wa.me/525551234567" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp"><i class="fab fa-whatsapp"></i></a>
                    </div>
                </div>

                <div class="footer-links">
                    <div class="footer-column">
                        <h4>Servicios</h4>
                        <a href="fabricacion.php">Fabricación</a>
                        <a href="soldadura.php">Soldadura</a>
                        <a href="catalogo.html">Catálogo</a>
                    </div>
                    <div class="footer-column">
                        <h4>Empresa</h4>
                        <a href="nosotros.php">Nosotros</a>
                        <a href="contacto.php">Contacto</a>
                        <a href="proyectos.php">Proyectos</a>
                    </div>
                    <div class="footer-column">
                        <h4>Legal</h4>
                        <a href="privacidad.php">Privacidad</a>
                        <a href="terminos.php">Términos</a>
                        <a href="garantias.php">Garantías</a>
                    </div>
                </div>
            </div>

            <div class="footer-bottom">
                <p>&copy; <?php echo date('Y'); ?> OSCON. Todos los derechos reservados.</p>
            </div>
        </div>
    </footer>

    <!-- Firebase Configuration -->
    <script type="module" src="assets/js/firebase-config.js"></script>

    <!-- Scripts -->
    <script src="assets/js/main.js"></script>
</body>

</html>
